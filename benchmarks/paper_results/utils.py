...
def prepare_dataframe(
    ...
    if sort_columns:
        result_df = result_df.sort_values(by=sort_columns).reset_index(drop=True)

    if verbose:
        print(f" -> Querying dataframe ({len(df)} rows) with query: '{query_str}'")
...
def calculate_scenario_variants_results(
    ...
    for _, sub_df in df.groupby(group_columns):
        ...
        if len(variants) == 2:
            line[(variants_name, "Diff")] = float(results[0]) - float(results[1])
        else:
...
def calculate_variant_wilcoxon_pandas(
    result_df: pd.DataFrame,
            ...
            values.append(
                {
                    ...
                    "Variant 1": (
                        " + ".join(map(str, v1_name))
                        if not isinstance(v1_name, str)
                        else v1_name
                    ),
                    "Variant 2": (
                        " + ".join(map(str, v2_name))
                        if not isinstance(v2_name, str)
                        else v2_name
                    ),
                    # "Variant 1 Mean": np.mean(result_v1),
                    # "Variant 2 Mean": np.mean(result_v2),
                    "Variant 1 Mean": result_v1.mean(),
                    "Variant 2 Mean": result_v2.mean(),

                    # "Variant 1 stdev": np.std(result_v1),
                    # "Variant 2 stdev": np.std(result_v2),
                    "Variant 1 stdev": result_v1.std(),   # sample std (ddof=1)
                    "Variant 2 stdev": result_v2.std(),

                    "P-Value": w_test.pvalue,
                    "Significant": (True if w_test.pvalue < threshold else False),
                }
            )
    ...
    if apply_bonferroni and len(raw_p_values) > 0:
        ...
        print(
            f"Bonferroni correction applied: α/{n_comparisons} = {adjusted_threshold:.6f}"
        )
            ...
            values.append(
                {
                    ...
                    # "Variant 1 stdev": result_v1.std(),   # sample std (ddof=1)
                    # "Variant 2 stdev": result_v2.std(),
                    "Variant 1 stdev": np.std(result_v1, ddof=1),
                    "Variant 2 stdev": np.std(result_v2, ddof=1),
                    "P-Value": w_test.pvalue,
                    "Significant": (True if w_test.pvalue < threshold else False),
                }
            )
...
def format_pvalue(p_value):
    ...
    # arredondamento (round half up)
    p_str = str(Decimal(p_value).quantize(Decimal("0.001"), rounding=ROUND_HALF_UP))
    return f"p={p_str}"
...
def create_precedence_graph(
    ...
    dot = graphviz.Digraph(comment="Precedence Graph", graph_attr={"rankdir": "TB"})
    ...
    for _, row in wilcoxon_df.iterrows():
        ...
        #         # dot.edge(variant1, variant2, label=f"p={p_value:.3e}")
        #         # label = f"p={p_value:.3e}" if p_value < 0.001 else f"p={p_value:.3f}"
        #         if p_value != 0:
        #             exponent = floor(log10(abs(p_value)))
        #             coeff = p_value / (10 ** exponent)
        #             label = f"p={coeff:.2f}×10^{exponent}"
        #         else:
        #             label = "p=0"
        #         dot.edge(variant1, variant2, label=label)
        ...
        #         # dot.edge(variant2, variant1, label=f"p={p_value:.3e}")
        #         if p_value != 0:
        #             exponent = floor(log10(abs(p_value)))
        #             coeff = p_value / (10 ** exponent)
        #             label = f"p={coeff:.2f}×10^{exponent}"
        ...
        #             label = "p=0"
        #         # label = f"p={p_value:.3e}" if p_value < 0.001 else f"p={p_value:.3f}"
        #         dot.edge(variant2, variant1, label=label)
        ...
        for value_idx, bar_value in enumerate(bar_values):
            for color_idx, bar_color in enumerate(bar_colors):
                for pattern_idx, bar_pattern in enumerate(bar_patterns):
                    ...
                    if bar_pattern_column:
                        sub_df = sub_df[sub_df[bar_pattern_column] == bar_pattern]
                    ...
                    y = sub_df[result_column].astype(float).mean()  # Mean result
                    y_std = (
                        sub_df[result_column].astype(float).std()
                    )  # Standard deviation
                    color = colors[value_idx % len(colors)]  # Color based on bar_value
                    pattern = patterns[
                    ...
                    unique_offsetgroup = f"{value_idx}-{color_idx}-{pattern_idx}"
    ...
    for value_idx, line_value in enumerate(line_values):
        for color_idx, line_color in enumerate(line_colors):
            for style_idx, line_style_value in enumerate(line_styles_values):
                ...
                if line_style_column:
                    sub_df = sub_df[sub_df[line_style_column] == line_style_value]
                ...
                y_std = sub_df[result_column].astype(float).std()  # Standard deviation
                color = colors[value_idx % len(colors)]  # Color based on line_value
                line_style = line_styles[style_idx % len(line_styles)]  # Line style
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        line=dict(color=color, dash=line_style),  # Line style and color
    ...
    for scenario_ID, scenario_df in sorted(df.groupby(group_scenarios_vars)):
        ...
        for variant_ID, variant_scenario_df in sorted(
            ...
            if variant_scenario_df.empty:
                ...
                continue
            # Extract values for plotting
            y = variant_scenario_df[result_column].astype(float).mean()  # Mean result
            y_std = (

...
def _load_weights(model, url, backbone_only):
    ...
    if any(k.startswith("fc.") for k in state):
        # finetuned checkpoint: backbone and head both come from the file
        model.load_state_dict(state, strict=True)
    else:
        # pretrained checkpoint: the SSL backbone loads, the head stays random,
        # which is the starting point for finetuning
